...
public static File CopyFile(File first, File second) throws IOException {

FileChannel fromDestination = null;
FileChannel toDestination = null;

try {
	fromDestination = new FileInputStream(first).getChannel();
	toDestination = new FileInputStream(second).getChannel();
	
	long counter = 0;
	long size = fromDestination.size();
	
	while((counter+= toDestination.transferFrom(fromDestination, counter, size - counter))<size);
	}
	finally {
		if(fromDestination != null) {
            fromDestination.close();
        }
...
public static String MoveObjToindex(Move move) {
	String fromRow = String.valueOf(move.from.row);
	String fromColumn = String.valueOf(move.from.column);
	String toRow = String.valueOf(move.to.row);
	String toColumn = String.valueOf(move.to.column);
...
public static void copy(Piece[][] pieces, Piece[][] update){ //RE-EVALUATE USE
	for(int i=0; i<update.length; i++) {
		for(int j=0; j<update[i].length; j++) {
			update[i][j]=pieces[i][j];
		}
	}	  
}

public static List<List<Piece>> convertArraytoList(Piece [][] pieces) {//convert pieces array to arraylist
	List<List<Piece>> lists = new ArrayList<>();
	for (Piece[] characters : pieces) {
	    List<Piece> list = new ArrayList<>();
	    for (Piece i : characters) {
	        list.add(i);
	    }
	    lists.add(list);
	}
...
public static Piece getPieceGUI(int row, int column, Piece[][] Pieces) {
